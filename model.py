...
class Model():
    def __init__(self, learning_rate=0.001, batch_size=3, num_steps=32, num_words=5000, dim_embedding=128, rnn_layers=3):
        r"""初始化函数
    ...
    def build(self, embedding_file=None):
        ...
        with tf.variable_scope('softmax'):
            softmax_W = tf.get_variable('W_o', [self.dim_embedding, self.num_words], initializer=tf.random_normal_initializer(stddev=0.01))
            softmax_b = tf.get_variable('b_o', [self.num_words], initializer=tf.constant_initializer(0.0))
